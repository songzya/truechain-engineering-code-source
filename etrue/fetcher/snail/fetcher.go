...
type bodyFilterTask struct {
	...
	fruits [][]*types.SnailBlock // Collection of fruits per block bodies
...
func (f *Fetcher) FilterBodies(peer string, fruits [][]*types.SnailBlock, signs [][]*types.PbftSign, time time.Time) ([][]*types.SnailBlock, [][]*types.PbftSign) {
	log.Trace("Filtering snail bodies", "peer", peer, "fts", len(fruits), "signs", len(signs), "number", signs[0][0].FastHeight)
			...
			for _, header := range task.headers {
				...
				if announce := f.fetching[hash]; announce != nil && announce.origin == task.peer && f.fetched[hash] == nil && f.completing[hash] == nil && f.queued[hash] == nil {
					...
					if f.getBlock(hash) == nil {
						...
						if header.FruitsHash == types.CalcFruitHash([]*types.SnailBlock{}) && header.SignHash == types.CalcSignHash([]*types.PbftSign{}) {
			...
			for i := 0; i < len(task.fruits) && i < len(task.signs); i++ {
				...
				for hash, announce := range f.completing {
					if f.queued[hash] == nil {
						ftnHash := types.CalcFruitHash(types.Fruits(task.fruits[i]))
